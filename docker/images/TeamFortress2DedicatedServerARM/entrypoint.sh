#!/usr/bin/env bash
# TF2_MODE=init    — downloads/updates the server via SteamCMD (anonymous login) and exits.
# TF2_MODE=server  — waits for installation and starts the server (default).
set -uo pipefail

APP_PATH="${APP_PATH:-/home/admin/app}"
STEAMCMD_DIR="$APP_PATH/steamcmd"
TF2_INSTALL_DIR="$APP_PATH/tf2server"
TF2_APPID=232250
READY_MARKER="$TF2_INSTALL_DIR/.ready"
TMUX_SESSION="tf2"

# ── init ─────────────────────────────────────────────────────────────────────
# TF2 is free-to-play, so its dedicated server can be downloaded with an
# anonymous SteamCMD login — no Steam account/credentials required.
if [ "${TF2_MODE:-server}" = "init" ]; then
    mkdir -p "$STEAMCMD_DIR" "$TF2_INSTALL_DIR"
    ...
    while [ "$ATTEMPT" -le "$MAX_ATTEMPTS" ]; do
        ...
        (cd "$STEAMCMD_DIR" && LD_LIBRARY_PATH="linux32" FEX linux32/steamcmd \
            +force_install_dir "$TF2_INSTALL_DIR" \
            +login anonymous \
            +app_update "$TF2_APPID" \
            +quit)

        if [ -f "$TF2_INSTALL_DIR/srcds_linux" ]; then
            SUCCESS=1
            break
        fi
...
# ── server ───────────────────────────────────────────────────────────────────
echo "==> Waiting for TF2 installation at $TF2_INSTALL_DIR..."
...
ln -sf "$TF2_INSTALL_DIR/bin/steamclient.so" "$HOME/.steam/sdk32/steamclient.so"

cp /opt/start-server.sh "$TF2_INSTALL_DIR/start-server.sh"
chmod +x "$TF2_INSTALL_DIR/start-server.sh"

STARTUP_LOG="/tmp/tf2-startup.log"
STARTUP_TIMEOUT="${TF2_STARTUP_TIMEOUT:-120}"

echo "==> Starting server on port ${TF2_PORT:-27015} in tmux session '$TMUX_SESSION'..."
> "$STARTUP_LOG"
tmux new-session -d -s "$TMUX_SESSION" -c "$TF2_INSTALL_DIR" "$TF2_INSTALL_DIR/start-server.sh"
...
SHUTDOWN_TIMEOUT="${TF2_SHUTDOWN_TIMEOUT:-30}"
...
# Attach to server console: docker exec -it <container> tmux attach -t tf2
