#!/usr/bin/env bash
IMAGE_NAME="teamfortress2dedicatedserverarm"
...
# ── Install/update TF2 via SteamCMD ─────────────────────────────────────────
# TF2 is free-to-play, so this uses an anonymous SteamCMD login — no Steam
# account or password is needed here.
DEFAULT_INIT_SERVICE="tf2-init"
read -p "SteamCMD init service name [$DEFAULT_INIT_SERVICE]: " INIT_SERVICE
INIT_SERVICE="${INIT_SERVICE:-$DEFAULT_INIT_SERVICE}"

echo "Installing/updating TF2, this may take a while..."
dc run --rm "$INIT_SERVICE" || { echo "TF2 installation failed, aborting."; exit 1; }
...
dc ps
echo "Attach to a server console: docker exec -it <container> tmux attach -t tf2"
